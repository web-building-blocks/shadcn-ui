...
interface TourComponentProps {
  ...
  buttonNames: { upload: string; save: string; others: string } // 新增按钮名称控件
}
...
const TourComponent: React.FC<TourComponentProps> = ({
  ...
  const getTourPopupPosition = () => {
    if (tourStep > 0 && stepRefs[tourStep - 1].current) {
      const rect = stepRefs[tourStep - 1].current.getBoundingClientRect()
      return {
