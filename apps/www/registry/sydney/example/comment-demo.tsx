  ...
  return (
    <>
      ...
      <Card className="mb-4" style={{ borderColor: "#EAEAEA" }}>
        <CardHeader className="flex flex-col p-4" style={{ borderBottom: "1px solid #EAEAEA" }}>
          <div className="flex items-center mb-2">
            <Avatar style={{ marginRight: "10px" }}>
              <AvatarImage src={message.avatarUrl} alt={message.user} />
            </Avatar>
            <div className="flex flex-col">
              <Label style={{ fontFamily: "Apercu Pro", fontWeight: "bold", fontSize: "1.05rem", color: "#333333" }}>
                {message.user}
              </Label>
            </div>
          </div>
          <div className="flex flex-col" style={{ marginLeft: "50px" }}>
            <Label style={{ fontFamily: "Apercu Pro", fontSize: "0.85rem", color: "gray" }}>
              {message.description}
            </Label>
          </div>
        </CardHeader>
        <CardContent>
          ...
          <div className="flex mt-2 justify-left text-sm space-x-2" style={{ color: "#666666", marginTop: "20px" }}>
            <Button variant="link" onClick={() => incrementStat("likes")} style={{ color: "#666666" }}>
              <Heart size={20} style={{ marginRight: "5px" }} />
              {message.likes}
            </Button>
            <span className="text-gray-300 mx-1" style={{ color: "#EAEAEA" }}>|</span>
            <Button variant="link" onClick={() => incrementStat("comments")} style={{ color: "#666666" }}>
              <MessageCircleMore size={20} style={{ marginRight: "5px" }} />
              {message.comments}
            </Button>
            <span className="text-gray-300 mx-1" style={{ color: "#EAEAEA" }}>|</span>
            <Button variant="link" onClick={() => incrementStat("shares")} style={{ color: "#666666" }}>
              <Forward size={20} style={{ marginRight: "5px" }} />
              {message.shares}
            </Button>
          </div>
          {isTextareaActive && (
            <div className="grid w-full gap-2 mt-2">
              ...
              />
              <div className="flex justify-end space-x-2">
                <Button onClick={() => setIsTextareaActive(false)} variant="secondary" style={{ fontFamily: "Apercu Pro" }}>
                  Close
                </Button>
                <Button onClick={postComment} variant="default" disabled={!commentText.trim()} style={{ fontFamily: "Apercu Pro", backgroundColor: "#FF6F00", color: "#FFFFFF" }}>
                  Post Comment
                </Button>
              </div>
            </div>
